import numpy as np   #改造二步三阶定步长 Robertson equation
import numpy.matlib
import matplotlib.pyplot as plt
from numpy.polynomial import chebyshev
import time
import copy
import math
import pandas as pd
'''
np.seterr(divide='ignore', invalid='ignore')
pi=math.pi
M=1000
time_st=time.time()
x0=0
x_end=1
x=np.linspace(x0,x_end,M+1,dtype=float)
hx=x[1]-x[0]
bt=0.1
af=0.1
gm=0.1
e=np.zeros((M-1,1))
BB=np.zeros((3*M-3,3*M-3))
A=np.zeros((M-1,M-1)) 
B=np.zeros((M-1,M-1)) 
C=np.zeros((M-1,M-1))
solu=np.zeros((3*M-3,1))
y=np.zeros((3*M-3,1))
atol=1e-5
rtol=1e-5
lop=440000
for i in range(0,M-1):
    if i==0:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
        e[0]=0
        B[0][0],B[0][1]=-2*bt/(hx**2),bt/(hx**2)
        A[0][0],A[0][1]=-2*af/(hx**2),af/(hx**2)
        C[0][0],C[0][1]=-2*gm/(hx**2),gm/(hx**2)

    elif 0<i<M-2:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
        e[i]=1
        B[i][i-1],B[i][i],B[i][i+1]=bt/(hx**2),-2*bt/(hx**2),bt/(hx**2)
        A[i][i-1],A[i][i],A[i][i+1]=af/(hx**2),-2*af/(hx**2),af/(hx**2)
        C[i][i-1],C[i][i],C[i][i+1]=gm/(hx**2),-2*gm/(hx**2),gm/(hx**2)
    else:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
        
        e[i]=0 
        B[M-2][M-2],B[M-2][M-3]=-2*bt/(hx**2),bt/(hx**2)
        A[M-2][M-2],A[M-2][M-3]=-2*af/(hx**2),af/(hx**2)
        C[M-2][M-2],C[M-2][M-3]=-2*gm/(hx**2),gm/(hx**2)
       

BB[0:M-1,0:M-1],BB[M-1:2*M-2,M-1:2*M-2]=A,B
BB[2*M-2:3*M-3,2*M-2:3*M-3]=C
print(BB)

def fun1(x,z):
    b=np.zeros((3*M-3,1))
    u=z[0:M-1]
    v=z[M-1:2*M-2]
    w=z[2*M-2:3*M-3]
    b5=np.zeros((3*M-3,1))
    b5[0][0]=af*M**2
    b5[M-2][0]=af*M**2
    for j in range(0,M-1):
        b[j]=(10**4)*v[j]*w[j]-0.04*u[j]
        b[M-1+j]=-3*(10**7)*(v[j]**2)-(10**4)*v[j]*w[j]+0.04*u[j]
        b[2*M-2+j]=3*(10**7)*(v[j]**2)
    b=b.reshape((3*M-3,1))
    U=np.dot(BB,z)+b5
    U=U.reshape((3*M-3,1))

    return U+b
'''
np.seterr(divide='ignore', invalid='ignore')
pi=math.pi
M=1000
time_st=time.time()
x0=0
x_end=1
x=np.linspace(x0,x_end,M+1,dtype=float)
hx=x[1]-x[0]
bt=0.1
af=0.1
gm=0.1
e=np.zeros((M-1,1))
BB=np.zeros((3*M-3,3*M-3))
A=np.zeros((M-1,M-1)) 
B=np.zeros((M-1,M-1)) 
C=np.zeros((M-1,M-1))
solu=np.zeros((3*M-3,1))
y=np.zeros((3*M-3,1))
atol=1e-5
rtol=1e-5
lop=445000
for i in range(0,M-1):
    if i==0:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
        
        B[0][0],B[0][1]=-2*bt/(hx**2),bt/(hx**2)
        A[0][0],A[0][1]=-2*af/(hx**2),af/(hx**2)
        C[0][0],C[0][1]=-2*gm/(hx**2),gm/(hx**2)

    elif 0<i<M-2:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
      
        B[i][i-1],B[i][i],B[i][i+1]=bt/(hx**2),-2*bt/(hx**2),bt/(hx**2)
        A[i][i-1],A[i][i],A[i][i+1]=af/(hx**2),-2*af/(hx**2),af/(hx**2)
        C[i][i-1],C[i][i],C[i][i+1]=gm/(hx**2),-2*gm/(hx**2),gm/(hx**2)
    else:
        y[i]=1+np.sin(2*pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=0
        
      
        B[M-2][M-2],B[M-2][M-3]=-2*bt/(hx**2),bt/(hx**2)
        A[M-2][M-2],A[M-2][M-3]=-2*af/(hx**2),af/(hx**2)
        C[M-2][M-2],C[M-2][M-3]=-2*gm/(hx**2),gm/(hx**2)
       

BB[0:M-1,0:M-1],BB[M-1:2*M-2,M-1:2*M-2]=A,B
BB[2*M-2:3*M-3,2*M-2:3*M-3]=C


def fun1(x,z):
    b=np.zeros((3*M-3,1))
    u=z[0:M-1]
    v=z[M-1:2*M-2]
    w=z[2*M-2:3*M-3]
 
    for j in range(0,M-1):
        if j==0:
          b[j]=(10**4)*v[j]*w[j]-0.04*u[j]+af/(hx**2)
          b[M-1+j]=-3*(10**7)*(v[j]**2)-(10**4)*v[j]*w[j]+0.04*u[j]
          b[2*M-2+j]=3*(10**7)*(v[j]**2)
        if 0<j<M-2:
            b[j]=(10**4)*v[j]*w[j]-0.04*u[j]
            b[M-1+j]=-3*(10**7)*(v[j]**2)-(10**4)*v[j]*w[j]+0.04*u[j]
            b[2*M-2+j]=3*(10**7)*(v[j]**2)
        elif j==M-2:
            b[j]=(10**4)*v[j]*w[j]-0.04*u[j]+af/(hx**2)
            b[M-1+j]=-3*(10**7)*(v[j]**2)-(10**4)*v[j]*w[j]+0.04*u[j]
            b[2*M-2+j]=3*(10**7)*(v[j]**2)
    b=b.reshape((3*M-3,1))
    U=np.dot(BB,z).reshape((3*M-3,1))

    return U+b

def err(x,y,tc,h):
    x1=x.reshape((3*M-3,1))
    y1=y.reshape((3*M-3,1))
    z1=12*(x1-y1)
    return 0.1*(z1+6*h*(fun1(tc+h,x1)+fun1(tc+h,y1)))

def ro(x,y):
    e=1e-12;ln=len(y)
    Rv=y.copy()
    for j in range(ln):
        if y[j]==0:
            Rv[j]=e/2
        else:
            Rv[j]=y[j]*(1+e/2)
    e=max(e,e*np.linalg.norm(Rv,ord=2))
    Rv1=y.copy()
    f1=fun1(x,Rv1) 
    f2=fun1(x,Rv)
    Rv1=Rv+e*(f1-f2)/(np.linalg.norm(f1-f2))
    Rv1=Rv1.reshape((ln,1))
    f1=fun1(x,Rv1)
    R=np.linalg.norm(f1-f2)/e
    Rr=R
    fg=R;fg1=0
    while fg > 1e-4*R and fg1<20:
        Rv1=Rv+e*(f1-f2)/np.linalg.norm(f1-f2)
        f1=fun1(x,Rv1)
        R=np.linalg.norm(f1-f2)/e
        fg=np.abs(R-Rr)
        fg1+=1
        Rr=R 
    if fg1==20:
        R=1.1*R
    return R,fg1

RKCv2= np.load(r'C:\Users\A204-7\Desktop\RKC\RKC\twostepRKCv4.npz', allow_pickle=True)
cs = RKCv2['cs']
us1=RKCv2['us1']
vs1=RKCv2['vs1']
vs=RKCv2['vs']
us=RKCv2['us']
yts=RKCv2['yts']

RKCv24= np.load(r'C:\Users\A204-7\Desktop\RKC\RKC\RKC2v1.npz', allow_pickle=True)
RKC2cs = RKCv24['cs']
RKC2us1=RKCv24['us1']
RKC2vs1=RKCv24['vs1']
RKC2vs=RKCv24['vs']
RKC2us=RKCv24['us']

def RKC2(fun1,t0,h,u0,s): 
    h1=h
    tc=[t0] #t的初始
    y1=u0 
    counter1=0
    while counter1==0:
        c=RKC2cs[s,0]
        u1=RKC2us1[s,0]
        u=RKC2us[s,0]
        v1=RKC2vs1[s,0]
        v=RKC2vs[s,0]
        k0=np.zeros((3*M-3,1))
        k1=np.zeros((3*M-3,1))
        k2=np.zeros((3*M-3,1))
        k3=np.zeros((3*M-3,1))
        ky0=np.zeros((3*M-3,1))
        ky1=np.zeros((3*M-3,1))
        k0=y1[:,-1].copy()
        k0=k0.reshape((3*M-3,1))
        ky0=fun1(tc[-1],k0)
        k1=k0+u1[1] *h *ky0
        ky1=fun1(tc[-1]+u1[1]*h,k1) 
        k2=k1.copy()
        k1=k0.copy()
        for j in range(2,s+1):

            k3=u[j]*k2+v[j]*k1+(1-u[j]-v[j])*k0+u1[j]*h*ky1+v1[j]*h*ky0
            #if j==4:
                #print(k[4])
            ky1=fun1(tc[-1]+c[j]*h,k3)
            k1=k2.copy()
            k2=k3.copy()
      
        
        yc=k3.copy()
            #err2=err(y[:,-1],yc,tc[-1],h1)
            #err1=np.linalg.norm(err2)/math.sqrt(2*M+2)
            #print(err1)
            # fac=0.8*((1/err1)**(1/3))
        counter1=1
    return yc



def RKC(fun1,t0,t_end,h,u0,s): 
    h1=h
    tc=[t0] #t的初始
    y=u0
    counter=0
    fg1=0
    nfe=0
    s_max=0
    y2=np.zeros((3*M-3,1))
    while tc[-1]<t_end:
        c=cs[s,0]
        u1=us1[s,0]
        u=us[s,0]
        v1=vs1[s,0]
        v=vs[s,0]
        yt=yts[s,0]
        if counter==1:
            h=yt*h1
        nfe=s+nfe+fg1+3
        k0=np.zeros((3*M-3,1))
        k1=np.zeros((3*M-3,1))
        k2=np.zeros((3*M-3,1))
        k3=np.zeros((3*M-3,1))
        ky0=np.zeros((3*M-3,1))
        ky1=np.zeros((3*M-3,1))
        
        k0=y.copy()
        k0=k0.reshape((3*M-3,1))
        ky0=fun1(tc[-1],k0)
        k1=k0+u1[1] *h *ky0
        ky1=fun1(tc[-1]+u1[1]*h,k1)
    
        k2=k1.copy()
        k1=k0.copy()
        for j in range(2,s+1):

            k3=u[j]*k2+v[j]*k1+(1-u[j]-v[j])*k0+u1[j]*h*ky1+v1[j]*h*ky0
            #if j==4:
                #print(k[4])
            ky1=fun1(tc[-1]+c[j]*h,k3)
            k1=k2.copy()
            k2=k3.copy()
        r=1
        bn=(1+r)/(yt*(1+r*yt))
        bf1=(r**2)*(1-yt)/(1+yt*r)
        b0=1-bn-bf1
        
        if counter==0:
            #yc=RKC2(fun1,t0,h,u0,s)
            yc=k3.copy()
            # print(yc)
            #err2=err(y[:,-1],yc,tc[-1],h1)
            #err1=np.linalg.norm(err2)/math.sqrt(2*M+2)
            #print(err1)
            # fac=0.8*((1/err1)**(1/3))
            y2=y.copy()
            y=yc.copy()
            
            counter=1
            pu,fg1=ro(tc[-1]+h1,yc)
            tc.append(tc[-1]+h1)
            s2=math.sqrt(h1*pu/0.4)
            s=math.ceil(s2)
            if s_max<s:
                   s_max=s
            if s<3:
                    s=3
            if s>250:
                s=250
            
         
            


           
        else :
            k02=y2.copy()
            k02=k02.reshape((3*M-3,1))
            yb=k3.copy()
        
            yc=bf1*k02+b0*k0+bn*yb
            pu,fg1=ro(tc[-1]+h1,yc)
            tc.append(tc[-1]+h1) 
            if tc[-1] + h1 > t_end:
                 h1 = t_end -tc[-1]
                
            s2=np.sqrt(h1*pu/0.4)                                           
            s=math.ceil(s2)
            if s<3:
                s=3
            if s>s_max:
                s_max=s
            if s>250:
                s=250  
            #h=h1
            y2=y.copy()
            y=yc.copy()

    return np.array(tc),np.array(y),np.array(y2),nfe,s_max
t0=0
t_end=1
h=0.02
eig3,fg1=ro(0,y)
s2=np.sqrt(h*eig3/0.4)                                           
s=math.ceil(s2)
print(s)
print('eig:',eig3)
#print(fun1(x,y))
#print(y)
if s<=3:
    s=3
#tc1,y1,nfe1,s_max1=RKC2(fun1,t0,t_end,0.0001,y,s)
tc,y,y2,nfe,s_max=RKC(fun1,t0,t_end,h,y,s)
#mse = np.mean((np.array(y[1:M,-1]) - np.array(solu[1:M]))**2)
#mae = np.mean(np.abs(np.array(y[1:M,-1]) - np.array(solu[1:M])
# ))
#err=sum([(x - y) ** 2 for x, y in zip(y[:,-2], y1[:,-2])] )/ len(y1[:,-2])
#print("terrpr:",np.sqrt(err))
time_end=time.time()
print(time_end-time_st)
print(tc)
print("步数：",len(tc))
print("评估次数：",nfe)
print("s_max:",s_max)
err2=err(y2,y,tc[-1],h)
#print(y[:,3])
err1=np.linalg.norm(err2)/math.sqrt(3*M+3)
print("err1:",err1)
solu1=np.load('SERKV2ROBsolu0.000001v2.npy')
#solu1=np.load(r'C:\Users\A204-7\Desktop\RKC\RKC\Robertsoneqsolu.npy')
err=sum([(x - y) ** 2 for x, y in zip(y[0:3*M-3,-1], solu1[0:3*M-3])] )/ len(solu1[0:3*M-3])
err3=sum([(x - y) ** 2 for x, y in zip(y2[0:3*M-3,-1], solu1[0:3*M-3])] )/ len(solu1[0:3*M-3])
print("err:",np.sqrt(err))
print("err3:",np.sqrt(err3))
