...
for i in range(0,M-1):
    if i==0:
        y[i]=np.sin(pi*x[i+1])
        y[M-1+i]=0
        y[2*M-2+i]=1-np.sin(pi*x[i+1])
        solu[i]=np.exp(-1)*np.sin(pi*x[i+1])
        solu[M-1+i]=0
        solu[2*M-2+i]=1-np.exp(-1)*np.sin(pi*x[i+1])
...
err=sum([(x - y) ** 2 for x, y in zip(y[0:3*M-3,-1], solu[0:3*M-3])] )/ len(solu[1:3*M-3])
