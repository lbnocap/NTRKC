...
def RKC(fun1,t0,t_end,h,u0,s): 
    ...
    return np.array(tc),np.array(y),np.array(y2),nfe,s_max,solu,solu1
t0=0
t_end=0.6
h=0.0000011
eig3,fg1=ro(0,y0)
...
MM=np.ceil(2/h)
MM=int(MM)
tc,y,y2,nfe,s_max,solu,solu1=RKC(fun1,t0,t_end,h,y0,s)
time_end=time.time()
...
df = pd.DataFrame({'VDPOLsolut_end0.6v1': Robsolu})
df1 = pd.DataFrame({'VDPOLsolut_end0.6v2': Robsolu1})

np.save('VDPOLsolut_end0.6v1.npy',Robsolu)
np.save('VDPOLsolut_end0.6v2.npy',Robsolu1)